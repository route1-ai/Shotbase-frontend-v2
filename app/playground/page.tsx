...
export default function Playground() {
  ...
  const [waitFor, setWaitFor] = useState('networkidle')
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#050505', color: '#f0f0f0' }}>
      <nav style={{ height: 56, borderBottom: '1px solid rgba(255,255,255,0.07)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 28px', flexShrink: 0, background: 'rgba(5,5,5,0.85)', backdropFilter: 'blur(20px)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          <Link href="/" aria-label="Shotbase Home" style={{ display: 'flex', alignItems: 'center', gap: 8, textDecoration: 'none' }}>
            <div style={{ width: 26, height: 26, background: '#00e87b', borderRadius: 6, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <svg width="13" height="13" viewBox="0 0 16 16" fill="none" aria-hidden="true"><rect x="1" y="1" width="14" height="10" rx="2" stroke="#000" strokeWidth="1.5"/><path d="M4 14h8M8 11v3" stroke="#000" strokeWidth="1.5" strokeLinecap="round"/></svg>
            </div>
      ...
      <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '360px 1fr', minHeight: 0 }}>
        <div style={{ borderRight: '1px solid rgba(255,255,255,0.07)', display: 'flex', flexDirection: 'column', overflow: 'auto', background: '#0a0a0a' }}>
          <div style={{ padding: '20px 20px 0', borderBottom: '1px solid rgba(255,255,255,0.07)', paddingBottom: 16 }}>
            <div style={{ fontFamily: 'var(--font-ibm-plex)', fontSize: 11, color: '#444', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 10 }}>Target URL</div>
          ...
          <div style={{ padding: 20, flex: 1 }}>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <div style={{ flex: 1 }}>
                  <div style={{ fontFamily: 'var(--font-ibm-plex)', fontSize: 10, color: '#444', marginBottom: 4 }}>Width</div>
                  <input type="number" value={width} onChange={e => setWidth(e.target.value)} style={{ width: '100%', fontFamily: 'var(--font-ibm-plex)', fontSize: 12, background: '#111', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 7, padding: '8px 10px', color: '#f0f0f0', outline: 'none' }}/>
                </div>
                <div style={{ flex: 1 }}>
                  <div style={{ fontFamily: 'var(--font-ibm-plex)', fontSize: 10, color: '#444', marginBottom: 4 }}>Height (auto)</div>
                  <input type="number" value={height} onChange={e => setHeight(e.target.value)} placeholder="auto" style={{ width: '100%', fontFamily: 'var(--font-ibm-plex)', fontSize: 12, background: '#111', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 7, padding: '8px 10px', color: '#f0f0f0', outline: 'none' }}/>
                </div>
              </div>
            </div>
          ...
          <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#050505', borderBottom: '1px solid rgba(255,255,255,0.07)', position: 'relative', minHeight: 0, overflow: 'hidden', padding: 24 }}>
            {!hasRun && (
              <div style={{ textAlign: 'center' }}>
                ...
                <div style={{ fontFamily: 'var(--font-ibm-plex)', fontSize: 13, color: '#444', marginBottom: 6 }}>No screenshot yet</div>
                <div style={{ fontFamily: 'var(--font-ibm-plex)', fontSize: 11, color: '#444' }}>Configure options and press Run</div>
              </div>
            )}
            {loading && (
              <div style={{ textAlign: 'center' }}>
                <div style={{ width: 48, height: 48, border: '2px solid rgba(255,255,255,0.07)', borderTopColor: '#00e87b', borderRadius: 24, animation: 'spin 0.8s linear infinite', margin: '0 auto 16px' }}/>
                <div style={{ fontFamily: 'var(--font-ibm-plex)', fontSize: 13, color: '#888' }}>Capturing screenshot…</div>
                <div style={{ fontFamily: 'var(--font-ibm-plex)', fontSize: 11, color: '#444', marginTop: 4 }}>Loading page, waiting for {waitFor}</div>
              </div>
            )}
            {result && !loading && (
              <React.Fragment>
                <img src={result.screenshotUrl} alt={`Screenshot of ${url}`} style={{ maxWidth: '100%', maxHeight: '100%', objectFit: 'contain', borderRadius: 8, border: '1px solid rgba(255,255,255,0.07)', boxShadow: '0 20px 60px rgba(0,0,0,0.5)' }}/>
          ...
          <div style={{ flexShrink: 0, borderTop: '1px solid rgba(255,255,255,0.07)', background: '#0a0a0a' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid rgba(255,255,255,0.07)', padding: '0 16px' }}>
              ...
              <button
                ...
                style={{ display: 'flex', alignItems: 'center', gap: 6, fontFamily: 'var(--font-ibm-plex)', fontSize: 11, color: copied ? '#00e87b' : '#444', background: 'none', border: 'none', cursor: 'pointer', padding: '6px 8px', transition: 'color 0.15s' }}
              >
