...
export default function WebhooksPage() {
  ...
  const copySnippet = async () => {
    if (!snippetRef.current?.textContent) return
    try {
      await navigator.clipboard.writeText(snippetRef.current.textContent)
    ...
    <div>
                  ...
                  <div aria-live="polite">
                    <button
                      ...
                      style={{
                        ...
                        fontSize: 11,
                        color: confirmDeleteId === ep.id ? "#ffffff" : "#ff6060",
                        background: confirmDeleteId === ep.id ? "#e53e3e" : "transparent",
                        border: `1px solid ${confirmDeleteId === ep.id ? "#e53e3e" : "rgba(255,60,60,0.2)"}`,
                        padding: "5px 12px",
                        borderRadius: 6,
                        cursor: "pointer",
      ...
      {endpoints.length > 0 && (
        <div style={{ ...cardStyle, marginTop: 20 }}>
          ...
          <pre ref={snippetRef} style={{ fontFamily: "var(--font-ibm-plex)", fontSize: 11, background: "#050505", border: `1px solid ${BORDER}`, padding: 12, borderRadius: 6, color: "#888", margin: 0, overflow: "auto", lineHeight: 1.65 }}>
