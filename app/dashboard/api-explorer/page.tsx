...
const METHOD_COLOR: Record<Endpoint["method"], string> = { POST: "#00e87b", GET: "#8a9eff", DELETE: "#ff6060" }

function CopyButton({ text }: { text: string }) {
  const [copied, setCopied] = useState(false)
  const copy = () => { navigator.clipboard.writeText(text); setCopied(true); setTimeout(() => setCopied(false), 2000) }
  return (
    <button type="button" onClick={copy} aria-label={copied ? "Copied!" : "Copy code"} style={{ position: "absolute", top: 8, right: 8, padding: 6, background: "rgba(10,10,10,0.8)", backdropFilter: "blur(4px)", border: `1px solid ${BORDER}`, borderRadius: 6, cursor: "pointer", color: copied ? "#00e87b" : "#666", display: "flex", transition: "all 0.2s" }}>
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    border: `1px solid ${active || focused ? ACTIVE_BORDER : "transparent"}`,
                    outline: focused ? `1px solid ${ACTIVE_BORDER}` : "none",
                    outlineOffset: -1,
                    borderRadius: 6,
                    padding: "8px 10px",
                    cursor: "pointer",
