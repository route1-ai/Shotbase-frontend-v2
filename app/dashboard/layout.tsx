...
function MobileDrawer({
  ...
  return createPortal(
    // `inert` when closed removes the whole drawer subtree from tab order, the
    // a11y tree, and pointer interaction — so CLOSED-drawer links/buttons are not
    // keyboard-focusable (aria-hidden + pointer-events alone would not do this).
    // The panel stays mounted so the open/close transition can still animate.
    <div inert={!open} style={{ position: "fixed", inset: 0, zIndex: 1000, pointerEvents: open ? "auto" : "none" }} aria-hidden={!open}>
      <div
        onClick={onClose}
