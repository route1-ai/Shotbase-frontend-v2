                  ...
                  <td style={{ padding: "14px 16px 14px 0" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }} aria-live="polite">
                      ...
                      <button
                        type="button"
                        onClick={() => {
                          const willReveal = !revealed[k.id]
                          if (!willReveal && copiedId === k.id) setCopiedId(null)
                          setRevealed((r) => ({ ...r, [k.id]: willReveal }))
                        }}
                        aria-label={revealed[k.id] ? `Hide secret key for ${k.name}` : `Show secret key for ${k.name}`}
                        title={revealed[k.id] ? "Hide secret key" : "Show secret key"}
                        style={{ fontFamily: "var(--font-ibm-plex)", fontSize: 11, color: "#888", background: "none", border: "none", cursor: "pointer", padding: "2px 4px" }}
                      >
                      ...
                      {revealed[k.id] && k.key && (
                        <button
                          ...
                          title={copiedId === k.id ? "Copied to clipboard!" : "Copy API key to clipboard"}
                          style={{ display: "inline-flex", alignItems: "center", gap: 4, fontFamily: "var(--font-ibm-plex)", fontSize: 11, color: copiedId === k.id ? "#00e87b" : "#888", background: "none", border: "none", cursor: "pointer", padding: "2px 4px" }}
                        >
