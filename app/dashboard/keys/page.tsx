...
export default function KeysPage() {
  ...
  const copyKey = (id: string, s: string) => {
                ...
                <tr key={k.id} style={{ borderBottom: i < keys.length - 1 ? "1px solid rgba(255,255,255,0.07)" : "none", opacity: k.active === false ? 0.4 : 1 }}>
                  ...
                  <td style={{ padding: "14px 16px 14px 0" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      ...
                      <button
                        onClick={() => {
                          const currentlyRevealed = revealed[k.id]
                          setRevealed((r) => ({ ...r, [k.id]: !r[k.id] }))
                          if (currentlyRevealed && copiedId === k.id) {
                            setCopiedId(null)
                      ...
                      {k.key && (
                        <div style={{ display: "inline-flex", alignItems: "center" }} aria-live="polite">
                          <button
                            onClick={() => copyKey(k.id, k.key!)}
                            style={{ fontFamily: "var(--font-ibm-plex)", fontSize: 10, color: copiedId === k.id ? "#00e87b" : "#888", background: "none", border: "none", cursor: "pointer", padding: 0, display: "flex", alignItems: "center", gap: 4 }}
                            aria-label={copiedId === k.id ? "Key copied to clipboard" : "Copy key to clipboard"}
                          >
                            <span>·</span><span>{copiedId === k.id ? "copied!" : "copy"}</span>
                          </button>
                        </div>
                      )}
                    </div>
