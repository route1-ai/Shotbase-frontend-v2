...
export default function KeysPage() {
  ...
  const handleCopy = (k: APIKey) => {
    navigator.clipboard.writeText(k.key!).then(() => {
      setCopiedId(k.id); if (timeoutRef.current) clearTimeout(timeoutRef.current);
                ...
                <tr key={k.id} style={{ borderBottom: i < keys.length - 1 ? "1px solid rgba(255,255,255,0.07)" : "none", opacity: k.active === false ? 0.4 : 1 }}>
                  ...
                  <td style={{ padding: "14px 16px 14px 0" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      ...
                      <button
                        onClick={() => { setRevealed((r) => ({ ...r, [k.id]: !r[k.id] })); setCopiedId(null); if (timeoutRef.current) clearTimeout(timeoutRef.current) }}
                      ...
                      {revealed[k.id] && k.key && (
                        <button className="ccopy" onClick={() => handleCopy(k)} style={{ background: "none", border: "none", cursor: "pointer", color: copiedId === k.id ? "#00e87b" : "#888", display: "flex", alignItems: "center", padding: 0, marginRight: 0 }} aria-label={copiedId === k.id ? "API key copied!" : "Copy API key to clipboard"} type="button">
