...
export default function KeysPage() {
  ...
  const toggleReveal = (id: string) => {
    ...
    const willBeRevealed = !isCurrentlyRevealed
    setRevealed((r) => ({ ...r, [id]: willBeRevealed }))
    if (!willBeRevealed && copiedId === id) {
      setCopiedId(null)
      if (timerRef.current) {
        clearTimeout(timerRef.current)
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }} aria-live="polite">
                      ...
                      {revealed[k.id] && k.key && (
                        ...
                        >
                          ...
                          <span style={{ fontSize: 9 }}>{copiedId === k.id ? "Copied" : "Copy"}</span>
                        </button>
                      )}
                    </div>
                  </td>
